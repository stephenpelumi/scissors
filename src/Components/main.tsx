...
const Main: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:2000/shorten-url', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ longUrl, domain, alias }),
      });
  ...
  return (
    <div>
      ...
      <main>
        ...
        <UrlForm/>
        {/* <form onSubmit={handleSubmit} className="url_form_background">
